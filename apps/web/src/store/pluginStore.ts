  ...
  persist(
    (set, get) => ({
      ...
      fetchPluginConfig: async (id) => {
        const config = await window.electron.invoke<any>(
          "PluginServices:fetchConfig",
          {
            pluginId: id,
          }
        );
      ...
      fetchPlugins: async (forceRefresh = false) => {
        ...
        const plugins = await window.electron.invoke<any>(
          "PluginServices:fetchAll"
        );
        console.log("🚀 ~ fetchPlugins: ~ plugins:", plugins);
        set({ plugins });
      },
      fetchPluginResources: async (id: string) => {
        const resources = await window.electron.invoke<any>(
          "PluginServices:fetchResources",
          {
            pluginId: id,
          }
        );
