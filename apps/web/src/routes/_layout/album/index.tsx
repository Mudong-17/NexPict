...
import { CloudTabList } from "@/components";
import { usePluginStore } from "@/store";
import { useEffect, useState } from "react";

const Album = () => {
  const pluginOptions = usePluginStore((state) => state.pluginOptions);
  const fetchPluginResources = usePluginStore(
    (state) => state.fetchPluginResources
  );
            ...
            {dataSource.map((item) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent h-16 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-between p-2">
                  <p className="text-white font-semibold truncate max-w-[calc(100%-40px)]">
                    {item.filename}
                  </p>
                  <div className="flex-shrink-0">
                    <Button
                      icon={
                        <Delete24Regular className="text-white group-hover:text-red-500" />
                      }
