...
import { useToast } from '@renderer/providers';
...
export default () => {
  ...
  const pluginConfigSchema = usePluginStore((state) => state.pluginConfigSchema);
  ...
  const saveConfig = async (data) => {
    const result: any = await window.electron.invoke('PluginServices:saveConfig', {
      pluginId: id,
      config: data,
    });
    if (result.code === 200) {
          ...
          <Controller
            ...
            render={({ field }) => {
              ...
              if (schema.type === 'boolean') {
                return (
                  <Field label={schema.label} required={schema.required} orientation="horizontal">
              ...
              if (schema.type === 'number') {
                return (
                  <Field label={schema.label} required={schema.required}>
                    <Input {...field} type="number" placeholder={schema.description} />
                  </Field>
                );
              }
